...
-- Insert 2027 event (idempotent via name guard)
...
-- Seed 267 booths against the 2027 event_id (idempotent via exists-check).
-- Schema reference: booths(event_id, booth_number text, size booth_size, is_corner boolean,
-- status booth_status default 'available'). booth_number is text and size (not booth_size)
-- is the column for the booth_size enum.
do $$
declare
  v_event_id uuid;
begin
